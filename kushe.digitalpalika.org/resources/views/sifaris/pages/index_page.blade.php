    ...
    <section id="nagarikta-pratilipi-content" class="mt-2">
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route($view_array['create_page_url']) }}" style="float:right;" class="btn_palika" id="add-new"><i class="fas fa-plus-square"></i> नयाँ</a>
...
{{--                            <th>सिफारिस मिती</th>--}}
                            ...
                            @foreach($view_array['index_table_title'] as $title)
                                <th>{{$title}}</th>
                            @endforeach
                        </tr>
                        </thead>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var table = $('.data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route($view_array['index_page_url']) }}",
                    type: 'GET',
                    data: function(d) {
                ...
                columns: [
                        @foreach($view_array['index_table_title'] as $data_title=>$title)
                            {
                                    data: '{{$data_title}}',
                                     name: '{{$data_title}}'
                            },
            ...
            $(document).on('click', '.delete-nibedan', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            type: "delete",
                            url: "{{ route($view_array['form_destroy_url'], '') }}" + "/" + id,
                            data: data,
